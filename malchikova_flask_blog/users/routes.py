...
from malchikova_flask_blog import db, bcrypt
from malchikova_flask_blog.models import User, Post
from malchikova_flask_blog.users.forms import RegistrationForm, LoginForm, \
    UpdateAccountForm, \
    RequestResetPasswordForm, ResetPasswordForm
from malchikova_flask_blog.users.utils import save_picture, send_reset_email
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('На вашу почту отправлено письмо с инструкциями по сбросу пароля.', 'info')
...
def reset_token(token):
    ...
    if user is None:
        ...
        return redirect(url_for('users.reset-request'))
